...
export const FriendsStoreModel = types
  ...
  .views((store) => ({
    ...
    getRequestById(id: string) {
      return store.friendRequests.find((request) => request._id === id)
    },
  }))
  .actions((store) => ({
  ...
  .actions((store) => ({
    ...
    acceptRequest: flow(function* acceptRequest(requestId: string) {
      const friendRequest = store.friendRequests.find((request) => request._id === requestId)
      if (!friendRequest) {
        return false
      }
      ...
      store.addFriend(
        FriendModel.create({
          _id: friendRequest.friendId,
          name: friendRequest.friendName,
          profImg: friendRequest.friendProfImg,
    ...
    denyRequest: flow(function* denyRequest(requestId: string) {
      const request = store.friendRequests.find((request) => request._id === requestId)
      if (request) {
